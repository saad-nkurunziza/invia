...
export const getOpeningClosingStockValues = unstable_cache(
  async (date: Date) => {
    try {
      const user = await getAuthenticatedUser();
      if (!user) return { status: "error", msg: "User not authenticated" };
...
export const fetchMostSoldProductsThisMonth = unstable_cache(
  async () => {
    try {
      const user = await getAuthenticatedUser();
      if (!user) return { status: "error", msg: "User not authenticated" };
...
export const fetchLeastSoldProductsThisMonth = unstable_cache(
  async () => {
    try {
      const user = await getAuthenticatedUser();
      if (!user) return { status: "error", msg: "User not authenticated" };
  ...
  ["least-sold-products-this-month"],
  { revalidate: 3600, tags: ["products"] }
);
